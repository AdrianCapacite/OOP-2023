    ...
    public void draw() {
        ...
        for (int i = 0; i < lines; i++) {
            ...
            for (int j = 0; j < sections; j++) {
                ...
                float displacement = map(noise(x * scale, y * scale, frameCount * timeMod), 0, 1, -100, 100);
                float mouseDist = dist(mouseX, mouseY, x, y);
                // y += displacement ;
                float j2 = map(j, 0, sections+1, -1, 1);
                float distToEdge = min(min(x, y), min(width-x, height-y));
                float dtemap = map(distToEdge, 0, min(width/2, height/2)/2, 1, 0);
                y += displacement * constrain(0-(dtemap * dtemap)*4+2,0,1);
